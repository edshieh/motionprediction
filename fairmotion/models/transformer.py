...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.5, max_len=5000):
        ...
        position = torch.arange(0, max_len, dtype=torch.float).double().unsqueeze(1)
        div_term = torch.exp(
            torch.arange(0, d_model, 2).double() * (-np.log(10000.0) / d_model)
        )
...
class TransformerLSTMModel(nn.Module):
    ...
    def forward(self, src, tgt, max_len=None, teacher_forcing_ratio=None):
        ...
        final_encoder_state = output[:, -1].unsqueeze(0)
        output = self.decoder(
            tgt,
